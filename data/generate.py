...
def _generate_session(scenario: Scenario, rng: random.Random, np_rng: np.random.Generator) -> SessionRecord:
    if scenario == "happy_path":
        ...
        cart_value = float(np.clip(np_rng.normal(120, 40), 20, 500))
        ...
        time_on_checkout = float(np.clip(np_rng.normal(90, 30), 10, 300))
        session_duration = float(np.clip(np_rng.normal(600, 120), 60, 1800))
        time_since_last = float(np.clip(np_rng.exponential(15), 1, 60))
        abandoned = 1 if rng.random() < 0.25 else 0

    elif scenario == "high_risk":
        device = rng.choices(DEVICE_TYPES, weights=[0.75, 0.2, 0.05])[0]
        cart_value = float(np.clip(np_rng.normal(35, 15), 5, 120))
        ...
        time_on_checkout = float(np.clip(np_rng.normal(20, 15), 0, 90))
        session_duration = float(np.clip(np_rng.normal(200, 80), 30, 600))
        time_since_last = float(np.clip(np_rng.exponential(120), 30, 600))
        ...
        if edge == "zero_cart":
            ...
            session_duration = float(np.clip(np_rng.exponential(60), 5, 300))
            time_since_last = float(np.clip(np_rng.exponential(30), 5, 300))
            abandoned = 1
        elif edge == "single_item":
            cart_value = float(np.clip(np_rng.normal(25, 10), 1, 100))
            item_count = 1
            cart_adds = 1
            cart_removes = 0
            page_views = rng.randint(1, 5)
            checkout_attempts = rng.randint(0, 1)
            time_on_checkout = float(np.clip(np_rng.normal(30, 20), 0, 120))
            session_duration = float(np.clip(np_rng.normal(180, 60), 30, 600))
            time_since_last = float(np.clip(np_rng.exponential(60), 5, 300))
            abandoned = 1 if rng.random() < 0.65 else 0
        elif edge == "long_session":
            cart_value = float(np.clip(np_rng.normal(200, 60), 50, 800))
            ...
            time_on_checkout = float(np.clip(np_rng.normal(300, 100), 60, 900))
            session_duration = float(np.clip(np_rng.normal(3600, 600), 1800, 7200))
            time_since_last = float(np.clip(np_rng.exponential(5), 1, 30))
            abandoned = 1 if rng.random() < 0.35 else 0
        else:  # repeated_checkout
            cart_value = float(np.clip(np_rng.normal(80, 30), 20, 300))
            item_count = rng.randint(2, 5)
            cart_adds = item_count
            cart_removes = 0
            page_views = rng.randint(8, 20)
            checkout_attempts = rng.randint(3, 8)
            time_on_checkout = float(np.clip(np_rng.normal(400, 100), 120, 900))
            session_duration = float(np.clip(np_rng.normal(900, 300), 300, 2400))
            time_since_last = float(np.clip(np_rng.exponential(10), 1, 60))
        ...
        cart_value = float(np.clip(np_rng.lognormal(4.0, 0.8), 5, 1000))
        ...
        checkout_attempts = rng.randint(0, 3)
        time_on_checkout = float(np.clip(np_rng.exponential(60), 0, 600))
        session_duration = float(np.clip(np_rng.lognormal(5.5, 0.9), 30, 3600))
        time_since_last = float(np.clip(np_rng.exponential(45), 1, 600))
        abandon_prob = 0.50
        if device == "mobile":
            abandon_prob += 0.20
        if cart_value < 30:
        ...
        if checkout_attempts == 0:
            abandon_prob += 0.25
        elif checkout_attempts >= 2:
            abandon_prob -= 0.30
        if time_since_last > 300:
            abandon_prob += 0.15
        if page_views < 3:
            abandon_prob += 0.15
        abandoned = 1 if rng.random() < float(np.clip(abandon_prob, 0.05, 0.95)) else 0
